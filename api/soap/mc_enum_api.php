<?php
	...
	function mci_get_enum_id_from_objectref( $p_enum, $p_object_ref ) {
		if ( !is_null( $p_object_ref ) && array_key_exists( 'id', $p_object_ref ) && (int)$p_object_ref['id'] != 0 ) {
			$t_id =  (int)$p_object_ref['id'];
		} else {
			$t_enum = config_get( $p_enum . '_enum_string' );
			if ( !is_null( $p_object_ref ) && array_key_exists( 'name', $p_object_ref ) && !is_blank( $p_object_ref['name'] ) ) {
