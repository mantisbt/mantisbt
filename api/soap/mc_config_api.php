...
function mc_config_get_string( $p_username, $p_password, $p_config_var ) {
	...
	if( config_is_private( $p_config_var ) ) {
		return ApiObjectFactory::newSoapFault( 'Client', 'Access to \'' . $p_config_var . '\' is denied' );
	}

	if( !config_is_set( $p_config_var ) ) {
		return ApiObjectFactory::newSoapFault( 'Client', 'Config \'' . $p_config_var . '\' is undefined' );
	}
