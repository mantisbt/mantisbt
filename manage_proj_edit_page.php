...
<h4 class="widget-title lighter">
	...
	echo lang_get ( 'edit_project_title' )?>
</h4>
</div>
...
<div class="widget-main no-padding">
	<div class="table-responsive">
		...
		<fieldset>
			...
			<input type="hidden" id="project-id" name="project_id" value="<?php
			...
			<tr>
        		...
        		<td>
        			<select id="project-language" name="language" class="input-sm">
            			<?php
			...
			<tr>
				...
				<td>
					<input type="text" id="project-name" name="name" class="input-sm" size="60" maxlength="128" value="<?php

					echo string_attribute ( $t_row ['name'] )?>" />
				</td>
			</tr>
			<tr>
				<td class="category">
				...
				<td>
					<select id="project-status" name="status" class="input-sm">
						...
						print_enum_string_option_list ( 'project_status', ( int ) $t_row ['status'] )?>
					</select>
				</td>
			</tr>
			<tr>
				...
				<td>
					<label>
						<input type="checkbox" class="ace" id="project-enabled" name="enabled" <?php

						check_checked ( ( int ) $t_row ['enabled'], ON );
						?> />
						<span class="lbl"></span>
					</label>
				</td>
			...
			<tr>
				...
				<td>
					<label>
						<input type="checkbox" class="ace" id="project-inherit-global" name="inherit_global" <?php

						check_checked ( ( int ) $t_row ['inherit_global'], ON );
						?> />
						<span class="lbl"></span>
					</label>
				</td>
			...
			<tr>
				...
				<td>
					<select id="project-view-state" name="view_state" class="input-sm">
						...
						print_enum_string_option_list ( 'view_state', ( int ) $t_row ['view_state'] )?>
					</select>
				</td>
			</tr>
			<?php
			...
			if (file_is_uploading_enabled () && DATABASE !== config_get ( 'file_upload_method' )) {
				...
				echo lang_get ( 'upload_file_path' )?>
					...
					<td>
						<input type="text" id="project-file-path" name="file_path" class="input-sm" size="60" maxlength="250" value="<?php
			...
			<tr>
				...
				<td>
					<textarea class="form-control" id="project-description" name="description" cols="70" rows="5"><?php

					echo string_textarea ( $t_row ['description'] )?></textarea>
				</td>
			</tr>

			<?php

			event_signal ( 'EVENT_MANAGE_PROJECT_UPDATE_FORM', array (
					$f_project_id
			) );
			?>
		</fieldset>
		...
		<div class="widget-toolbox padding-8 clearfix">
			<span class="required pull-right"> * <?php

			echo lang_get ( 'required' )?></span>
...
<div id="project-delete-div" class="form-container">
	...
	echo form_security_field ( 'manage_proj_delete' )?>
			<input type="hidden" name="project_id" value="<?php

	echo $f_project_id?>" />
			<input type="submit" class="btn btn-primary btn-sm btn-white btn-round"" value="<?php

	echo lang_get ( 'delete_project_button' )?>" />
		</fieldset>
	</form>
</div>
<?php
...
<div class="col-md-12 col-xs-12">
	...
	if (access_has_global_level ( config_get ( 'create_project_threshold' ) )) {
		print_form_button ( 'manage_proj_create_page.php?parent_id=' . $f_project_id, lang_get ( 'create_new_subproject_link' ), null, null, 'btn btn-sm btn-primary btn-white btn-round' );
	}
	?>
	</div>
...
<?php
	...
	<div class="widget-box widget-color-blue2">
		...
		echo lang_get ( 'name' )?></th>
						<th><?php

		echo lang_get ( 'status' )?></th>
						<th><?php

		echo lang_get ( 'enabled' )?></th>
						<th><?php

		echo lang_get ( 'inherit' )?></th>
						<th><?php

		echo lang_get ( 'view_status' )?></th>
						<th><?php

		echo lang_get ( 'description' )?></th>
						<th colspan="2"><?php

		echo lang_get ( 'actions' )?></th>
					</tr>
				</thead>
				<tbody>
<?php
		foreach ( $t_subproject_ids as $t_subproject_id ) {
			...
			echo get_enum_element ( 'project_status', $t_subproject ['status'] )?>
						</td>
						<td class="center">
							<?php

			echo trans_bool ( $t_subproject ['enabled'] )?>
						</td>
						<td class="center">
						<label>
							<input type="checkbox" class="ace" name="inherit_child_<?php

			echo $t_subproject_id?>"
								<?php

			echo ($t_inherit_parent ? 'checked="checked"' : '')?>  />
								<span class="lbl"></span>
						</label>
						</td>
						<td class="center">
			...
			print_link_button ( 'manage_proj_edit_page.php?project_id=' . $t_subproject ['id'], lang_get ( 'edit_link' ), 'btn-xs' );
			?>
							<?php

			print_link_button ( "manage_proj_subproj_delete.php?project_id=$f_project_id&subproject_id=" . $t_subproject ['id'] . form_security_param ( 'manage_proj_subproj_delete' ), lang_get ( 'unlink_link' ), 'btn-xs' );
			?>
							</div>
						</td>
					</tr>
...
<div class="col-md-12 col-xs-12">
	...
	<form id="manage-project-category-copy-form" method="post" action="manage_proj_cat_copy.php" class="form-inline">
		<fieldset>
			...
			echo form_security_field ( 'manage_proj_cat_copy' )?>
			<input type="hidden" name="project_id" value="<?php
			...
			<input type="submit" name="copy_from" class="btn btn-sm btn-primary btn-white btn-round" value="<?php

			echo lang_get ( 'copy_categories_from' )?>" />
			<input type="submit" name="copy_to" class="btn btn-sm btn-primary btn-white btn-round" value="<?php

			echo lang_get ( 'copy_categories_to' )?>" />
		</fieldset>
	</form>
	</div>
		<div class="widget-body">
...
if (count ( $t_categories ) > 0) {
	...
	echo lang_get ( 'category' )?></th>
				<th><?php

	echo lang_get ( 'assign_to' )?></th>
...
<?php
	foreach ( $t_categories as $t_category ) {
		...
		echo string_display ( category_full_name ( $t_id, $t_inherited, $f_project_id ) )?></td>
				<td><?php

		echo prepare_user_name ( $t_category ['user_id'] )?></td>
				<td class="center">
					<div class="inline">
					<?php
		...
		if (! $t_inherited) {
			...
			print_form_button ( 'manage_proj_cat_edit_page.php?id=' . $t_id . '&project_id=' . $t_project_id, lang_get ( 'edit_link' ), null, null, 'btn btn-xs btn-primary btn-white btn-round' );
			echo '</div>';
		}
		?>
					<?php
		...
		if (! $t_inherited) {
			echo '<div class="pull-left">';
			print_form_button ( 'manage_proj_cat_delete.php?id=' . $t_id . '&project_id=' . $t_project_id, lang_get ( 'delete_link' ), null, null, 'btn btn-xs btn-primary btn-white btn-round' );
			echo '</div>';
		}
		?>
					</div>
...
<div class="widget-toolbox padding-8 clearfix">
	<form id="project-add-category-form" method="post" action="manage_proj_cat_add.php" class="form-inline">
		<fieldset>
			...
			<input type="submit" name="add_and_edit_category" class="btn btn-sm btn-primary btn-white btn-round" value="<?php

			echo lang_get ( 'add_and_edit_category_button' )?>" />
		</fieldset>
	</form>
</div>
</div>
...
<div id="project-versions-div" class="form-container">
	...
	<form id="manage-project-version-copy-form" method="post" action="manage_proj_ver_copy.php" class="form-inline">
		<fieldset>
			...
			echo form_security_field ( 'manage_proj_ver_copy' )?>
			<input type="hidden" class="form-control input-sm" name="project_id" value="<?php
			...
			echo lang_get ( 'copy_versions_from' )?>" />
			<input type="submit" name="copy_to" class="btn btn-sm btn-primary btn-white btn-round" value="<?php

			echo lang_get ( 'copy_versions_to' )?>" />
		</fieldset>
	</form>
	</div>
		<div class="widget-body">
	...
	<div class="table-responsive">
		...
		echo lang_get ( 'version' )?></th>
				<th><?php

		echo lang_get ( 'released' )?></th>
				<th><?php

		echo lang_get ( 'obsolete' )?></th>
				<th><?php

		echo lang_get ( 'timestamp' )?></th>
				<th><?php
...
<?php
		foreach ( $t_versions as $t_version ) {
			...
			echo string_display ( $t_name )?></td>
				<td class="center"><?php

			echo trans_bool ( $t_released )?></td>
				<td class="center"><?php

			echo trans_bool ( $t_obsolete )?></td>
				<td class="center"><?php

			echo $t_date_formatted?></td>
				<td class="center">
					<div class="inline">
					<?php
...
<?php
	...
	<form id="manage-project-add-version-form" method="post" action="manage_proj_ver_add.php" class="form-inline">
		<fieldset>
			...
			echo form_security_field ( 'manage_proj_ver_add' )?>
			<input type="hidden" name="project_id" value="<?php

			echo $f_project_id?>" />
			<input type="text" class="input-sm" name="version" size="32" maxlength="64" />
			<input type="submit" name="add_version" class="btn btn-sm btn-primary btn-white btn-round" value="<?php

			echo lang_get ( 'add_version_button' )?>" />
			<input type="submit" name="add_and_edit_version" class="btn btn-sm btn-primary btn-white btn-round" value="<?php

			echo lang_get ( 'add_and_edit_version_button' )?>" />
		</fieldset>
	</form>
	</div>
	</div>
...
if (access_has_project_level ( config_get ( 'custom_field_link_threshold' ), $f_project_id ) && ($t_custom_field_count > 0)) {
	?>
<div class="col-md-12 col-xs-12">
<div class="space-10"></div>
...
<div class="widget-box widget-color-blue2">
	...
	echo form_security_field ( 'manage_proj_custom_field_copy' )?>
			<input type="hidden" name="project_id" value="<?php
	...
	echo lang_get ( 'copy_from' )?>" />
			<input type="submit" name="copy_to" class="btn btn-sm btn-primary btn-white btn-round" value="<?php

	echo lang_get ( 'copy_to' )?>" />
		</fieldset>
	</form>
	</div>
<?php
	...
	<table class="table table-striped table-bordered table-condensed">
		...
		echo lang_get ( 'custom_field' )?></th>
				<th><?php

		echo lang_get ( 'custom_field_sequence' )?></th>
				<th><?php
...
<?php
		foreach ( $t_custom_fields as $t_field_id ) {
			...
			echo '<a href="manage_custom_field_edit_page.php?field_id=' . $t_field_id . '">' . custom_field_get_display_name ( $t_desc ['name'] ) . '</a>'?></td>
				<td class="center">
			...
			echo $t_field_id?>" />
							<input type="text" class="input-sm" name="sequence" value="<?php

			echo custom_field_get_sequence ( $t_field_id, $f_project_id )?>" size="2" />
							<input type="submit" class="btn btn-sm btn-primary btn-white btn-round" value="<?php
...
event_signal ( 'EVENT_MANAGE_PROJECT_PAGE', array (
		$f_project_id
) );
?>
	...
	<table class="table table-striped table-bordered table-condensed">
		<thead>
			<tr>
				...
				echo lang_get ( 'username' )?></th>
				<th><?php

				echo lang_get ( 'email' )?></th>
				<th><?php

				echo lang_get ( 'access_level' )?></th>
				<th><?php
...
for($i = 0; $i < $t_users_count; $i ++) {
	...
	echo get_enum_element ( 'access_levels', $t_user ['access_level'] )?></td>
				<td class="center"><?php
	...
	if ($t_can_manage_users && access_has_project_level ( $t_user ['access_level'], $f_project_id )) {
		if (! $f_show_global_users || $f_show_global_users && isset ( $t_local_users [$t_user ['id']] )) {
			print_form_button ( 'manage_proj_user_remove.php', lang_get ( 'remove_link' ), array (
...
// from this project
if (! $f_show_global_users) {
	print_form_button ( "manage_proj_edit_page.php?project_id=$f_project_id&show_global_users=true", lang_get ( 'show_global_users' ), null, OFF, 'btn btn-sm btn-primary btn-white btn-round' );
} else {
	print_form_button ( "manage_proj_edit_page.php?project_id=$f_project_id", lang_get ( 'hide_global_users' ), null, OFF, 'btn btn-sm btn-primary btn-white btn-round' );
}

if ($t_removable_users_exist) {
	echo '&#160;';
	print_form_button ( 'manage_proj_user_remove.php', lang_get ( 'remove_all_link' ), array (
			'project_id' => $f_project_id
	), $t_token_remove_user );
}
?>
