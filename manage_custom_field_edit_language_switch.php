...
/**
 * Loads localized custom field values for the selected language. If there
...
function update_custom_field() {
	...
	$localization = array( 'name' => 'ERROR', 'possible_values' => 'ERROR', 'default_value' => 'ERROR' );
	...
	if( isset( $_GET['id'] ) && isset( $_GET['lang'] ) ) {
		...
		if( strcmp( $t_lang, 'default' ) !== 0 ) {
			$t_query = "
                SELECT  name, possible_values, default_value
		...
		$t_pval = (!isset( $row['possible_values'] ) ? '' : $row['possible_values']);
		$t_dval = (!isset( $row['default_value'] ) ? '' : $row['default_value']);
		
		$localization['name'] = $t_name;
		$localization['possible_values'] = $t_pval;
		$localization['default_value'] = $t_dval;
	}
	return $localization;
}
