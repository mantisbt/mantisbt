...
foreach( $t_extensions_required as $t_extension ) {
	check_print_test_row(
		...
		[ false => 'MantisBT requires the ' . $t_extension . ' extension to either be compiled into PHP or loaded as an extension.' ]
	);
}
...
if( config_get_global( 'allow_file_upload' ) ) {
	check_print_test_row(
		...
		$t_fileinfo_loaded,
		[ false => 'Ensure that the fileinfo extension is installed and enabled' ]
	);
} else {
...
if ( $t_fileinfo_loaded ) {
	...
	check_print_test_row(
		...
		$t_finfo !== false,
		[ false => 'Ensure that the fileinfo_magic_db_file configuration value points to a valid magic.db file.' ]
	);
}
...
check_print_test_warn_row(
	...
	[ false => 'For security reasons this extension should not be loaded on production and Internet facing servers.' ]
);
...
check_print_test_row(
	...
	[ false => 'The value of this directive is currently: ' . $t_variables_order ]
);

check_print_test_warn_row(
	'register_argc_argv php.ini directive is disabled',
	!ini_get_bool( 'register_argc_argv' ),
	[ false => 'This directive should be disabled to increase performance (it only affects PHP in CLI mode).' ]
);

check_print_test_warn_row(
	'register_long_arrays php.ini directive is disabled',
	!ini_get_bool( 'register_long_arrays' ),
	[ false => 'This directive is deprecated in PHP 5.3.0 and should be disabled for performance reasons.' ]
);

check_print_test_warn_row(
	'auto_globals_jit php.ini directive is enabled',
	ini_get_bool( 'auto_globals_jit' ),
	[ false => 'This directive is currently disabled: enable it for a performance gain.' ]
);

check_print_test_warn_row(
	'display_errors php.ini directive is disabled',
	!ini_get_bool( 'display_errors' ),
	[ false => 'For security reasons this directive should be disabled on all production and Internet facing servers.' ]
);
...
check_print_test_warn_row(
	...
	[ false => 'PHP is not currently set to log or report errors and thus you may be unaware of PHP errors that occur.' ]
);
...
check_print_test_row(
	...
	[ false => 'The current value of the memory_limit directive is ' . htmlentities( ini_get_number( 'memory_limit' ) ) . ' bytes. This value needs to be at least equal to the post_max_size directive value of ' . htmlentities( ini_get_number( 'post_max_size' ) ) . ' bytes.' ]
);
...
check_print_test_row(
	...
	[ false => 'The current value of the post_max_size directive is ' . htmlentities( ini_get_number( 'post_max_size' ) ) . ' bytes. This value needs to be at least equal to the upload_max_filesize directive value of ' . htmlentities( ini_get_number( 'upload_max_filesize' ) ) . ' bytes.' ]
);
...
check_print_test_warn_row(
	...
	[ false => 'Please ensure web server configuration sets SCRIPT_NAME' ]
);
