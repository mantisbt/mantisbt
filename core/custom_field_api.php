 ...
 * @param boolean $p_trigger_errors Indicates whether to trigger an error if the
 * field is not found.
...
function custom_field_cache_array_rows( array $p_cf_id_array ) {
	...
	SELECT
	a.id,
	IF (b.name IS NULL, a.name, b.name) name,
	a.type,
	IF (b.possible_values IS NULL, a.possible_values, b.possible_values) possible_values,
	IF (b.default_value IS NULL, a.default_value, b.default_value) default_value,
	a.valid_regexp,
	a.access_level_r,
	a.access_level_rw,
	a.length_min,
	...
	ON a.id = b.id AND text_lang = " . db_param() . "
    WHERE a.id IN (" . implode( ",", $t_in_caluse_dbparams ) . ")
	";
 ...
 * When a value for a given bug and field does not exist, fill the cached value
 * as null
...
function custom_field_cache_values( array $p_bug_id_array, array $p_field_id_array ) {
	...
	$t_params = array();
	$t_query = '
        SELECT B.id AS bug_id, CF.id AS field_id, CFS.value, CFS.text FROM {bug} B
        LEFT OUTER JOIN {custom_field} CF ON 1 = 1
        LEFT OUTER JOIN {custom_field_ref} CFS
        ON ( B.id = CFS.bug_id AND CF.id = CFS.field_id )
    ';
	...
	while ( $t_row = db_fetch_array( $t_result ) ) {
		...
		$t_value = cp_get_localized_custom_field_value( $t_row[$t_value_column] );
		if( null !== $t_value ) {
...
/**
 * Clear the cache of custom field references in the bug (or all bugs).
 * @param unknown $p_bug_id Bug id
 * @return void
 */
function custom_field_clear_cache_refs( $p_bug_id = null ) {
	global $g_cache_cf_bug_refs;
	...
	if( null === $p_bug_id ) {
		...
		if( isset( $g_cache_cf_bug_refs[(int) $p_bug_id] ) ) {
			unset( $g_cache_cf_bug_refs[(int) $p_bug_id] );
		}
	}
}
...
/**
 * Return true if the user can read the value of the field for the given
 * project,
 * false otherwise.
 * @param integer $p_field_id Custom field identifier.
...
function custom_field_update( $p_field_id, array $p_def_array ) {
	...
	# Update localized custom fields in cp lang api!
	$t_translation = false;
	if( strcmp( $p_def_array['language'], 'default' ) != 0 ) {
		cp_create_custom_field_localisation( $p_field_id, $p_def_array['language'], $p_def_array['name'], $p_def_array['possible_values'], $p_def_array['default_value'] );
		$t_translation = true;
	}
...
function custom_field_destroy( $p_field_id ) {
	# delete all values
	db_param_push();
	$t_query = 'DELETE FROM {custom_field_ref} WHERE field_id=' . db_param();
...
function custom_field_delete_all_values( $p_bug_id ) {
	db_param_push();
	$t_query = 'DELETE FROM {custom_field_ref} WHERE bug_id=' . db_param();
 ...
 *
 * The ids will be sorted based on the sequence number associated with the
 * binding
 * @param integer $p_project_id A project identifier.
 * @return array
 * @access public
...
function custom_field_get_linked_ids( $p_project_id = ALL_PROJECTS ) {
	...
	if( !isset( $g_cache_cf_linked[$p_project_id] ) ) {
		...
		if( ALL_PROJECTS == $p_project_id ) {
			...
			# Select only the ids of custom fields in projects the user has
			# access to
...
function custom_field_get_ids() {
	...
	if( $g_cache_cf_list === null ) {
		...
		while ( $t_row = db_fetch_array( $t_result ) ) {
			# Disabled for value localisation
			# TODO: Change the query and uncomment
...
/**
 * Return a single database field from a custom field definition row for the
 * field
 ...
 * @param integer $p_field_id Custom field id.
 * @param integer $p_bug_id A bug identifier.
 * @return mixed: value is defined, null: no value is defined, false: read
 * access is denied
 * @access public
 */
function custom_field_get_value( $p_field_id, $p_bug_id ) {
	...
	# A null value means a cached non existant value. It must be checked with
	# care.
	if( !isset( $g_cache_cf_bug_values[$c_bug_id] ) || !array_key_exists( $c_field_id, $g_cache_cf_bug_values[$c_bug_id] ) ) {
...
/**
 * Get the original reference for the custom field in this bug.
 * @param integer $p_field_id Custom field id.
 * @param integer $p_bug_id A bug identifier.
 * @return mixed: ref is defined, null: no ref is defined
 * @access public
 */
function custom_field_get_ref( $p_field_id, $p_bug_id ) {
	global $g_cache_cf_bug_refs;
	$c_bug_id = (int) $p_bug_id;
	$c_field_id = (int) $p_field_id;
	
	if( !isset( $g_cache_cf_bug_refs[$c_bug_id] ) || !array_key_exists( $c_field_id, $g_cache_cf_bug_refs[$c_bug_id] ) ) {
		custom_field_cache_values( array( $c_bug_id ), array( $c_field_id ) );
	}
	
	return $g_cache_cf_bug_refs[$c_bug_id][$c_field_id];
}

/**
 * Gets the custom fields array for the given bug readable by specified level.
 * Array keys are custom field names. Array is sorted by custom field sequence
 * number;
...
/**
 * Gets the custom fields array for the given bug.
 * Array keys are custom field names.
 * Array is sorted by custom field sequence number; Array items are arrays with
 * the next keys:
 * 'type', 'value', 'access_level_r'
 * @param integer $p_bug_id A bug identifier.
 * @return array
...
function custom_field_get_all_linked_fields( $p_bug_id ) {
	...
	if( !array_key_exists( $p_bug_id, $g_cached_custom_field_lists ) ) {
		...
		while ( $t_row = db_fetch_array( $t_result ) ) {
			...
			$t_custom_fields[$t_row['name']] = array( 'type' => $t_row['type'], 'value' => $t_value, 'access_level_r' => $t_row['access_level_r'] );
		}
...
function custom_field_validate( $p_field_id, $p_value ) {
	...
	switch ( $t_type ) {
		...
		case CUSTOM_FIELD_TYPE_DATE:
			...
			# For 32 bit systems, supported range will be 13 Dec 1901 20:45:54
			# UTC to 19 Jan 2038 03:14:07 UTC
			$t_valid &= $p_value !== false;
			break;
		case CUSTOM_FIELD_TYPE_CHECKBOX:
		case CUSTOM_FIELD_TYPE_MULTILIST:
			# Checkbox fields can hold a null value (when no checkboxes are
			# ticked)
			if( $p_value === '' ) {
				break;
			}
		...
		case CUSTOM_FIELD_TYPE_RADIO:
			...
			$t_valid &= in_array( $p_value, $t_values_arr ) || cp_is_custom_field_localisation_key( $p_value );
			break;
...
/**
 * Convert the default-value to value depending on the type.
 * For example, in case of date, this
...
function custom_field_set_value( $p_field_id, $p_bug_id, $p_value, $p_log_insert = true ) {
	...
	# Determine whether an existing value needs to be updated or a new value
	# inserted
	db_param_push();
	$t_query = 'SELECT ' . $t_value_field . '
				  FROM {custom_field_ref}
				  WHERE field_id=' . db_param() . ' AND
				  		bug_id=' . db_param();
	...
	if( $t_row = db_fetch_array( $t_result ) ) {
		db_param_push();
		$t_query = 'UPDATE {custom_field_ref}
					  SET ' . $t_value_field . '=' . db_param() . '
					  WHERE field_id=' . db_param() . ' AND
					  		bug_id=' . db_param();
		$t_params = array( $t_value, (int) $p_field_id, (int) $p_bug_id );
		db_query( $t_query, $t_params );
		
		history_log_event_direct( $p_bug_id, $t_name, custom_field_database_to_value( $t_row[$t_value_field], $t_type ), $t_value );
	} else {
		db_param_push();
		$t_query = 'INSERT INTO {custom_field_ref}
						( field_id, bug_id, ' . $t_value_field . ' )
					  VALUES
						( ' . db_param() . ', ' . db_param() . ', ' . db_param() . ')';
		$t_params = array( (int) $p_field_id, (int) $p_bug_id, $t_value );
		db_query( $t_query, $t_params );
		# Don't log history events for new bug reports or on other special
		# occasions
		if( $p_log_insert ) {
 ...
 * field id
 * $p_bug_id contains the bug where this field belongs to.
 * If it's left
...
function print_custom_field_input( array $p_field_def, $p_bug_id = null ) {
	if( null === $p_bug_id ) {
		...
		if( $p_field_def['type'] == CUSTOM_FIELD_TYPE_RADIO ) {
			$t_custom_field_value = custom_field_get_ref( $p_field_def['id'], $p_bug_id );
		}
		# If the custom field value is undefined and the field cannot hold a
		# null value, use the default value instead
...
/**
 * Constructs the name of a field used as a flag to indicate that a custom field
 * is present on the form
 * @param integer $p_custom_field_id The custom field id to create the field
 * name for.
 ...
 * @param integer $p_bug_id Contains the bug id to display the custom field
 * value for.
 * @return string
 * @access public
 */
