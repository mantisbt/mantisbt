...
echo lang_get('tag_attach')?>" class="btn btn-primary btn-sm btn-white btn-round" />
	</form>
<?php
    return true;
}
...
echo config_get('tag_separator')?>" />
	<input type="text" name="tag_string" id="tag_string" class="input-sm" size="40" value="<?php

echo string_attribute($p_string)?>" />
	<select class="input-sm" <?php

echo helper_get_tab_index()?> name="tag_select" id="tag_select" class="input-sm">
		<?php
...
function print_news_item_option_list()
{
    ...
    $t_result = db_query($t_query, ($t_global == true ? array () : array (
        $t_project_id
    )));
...
function print_project_user_list($p_user_id, $p_include_remove_link = true)
{
    ...
    foreach ($t_projects as $t_project_id => $t_project) {
        ...
        if ($p_include_remove_link && access_has_project_level(config_get('project_user_threshold'), $t_project_id)) {
            html_button('manage_user_proj_delete.php', lang_get('remove_link'), array (
...
function print_bug_attachment_header(array $p_attachment, $p_security_token)
{
    ...
    if ($p_attachment['exists']) {
        ...
        event_signal('EVENT_VIEW_BUG_ATTACHMENT', array (
            $p_attachment
        ));
    } else {
        print_file_icon($p_attachment['display_name']);
