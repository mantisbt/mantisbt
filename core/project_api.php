...
function project_cache_row( $p_project_id, $p_trigger_errors = true ) {
	...
	if( isset( $g_cache_project[(int) $p_project_id] ) ) {
		...
		if( isset( $g_cache_project_missing[(int) $p_project_id] ) ) {
			return false;
		}
	
	db_param_push();
	
	$t_params = array( cp_get_language(), $p_project_id );
	...
	$t_query = "
        SELECT
            ...
            a.access_min,
            a.file_path,
            IF (b.description IS NULL, a.description, b.description) description,
            a.category_id,
            a.inherit_global
        FROM {project} a
        LEFT JOIN {project_localized} b
...
function project_cache_array_rows( array $p_project_id_array ) {
	...
	# $t_query = 'SELECT * FROM {project} WHERE id IN (' . implode(',',
	# $c_project_id_array) . ')';
	# $t_result = db_query($t_query);
	
	$t_projects_found = array();
	for ( $i = 0; $i < sizeof( $c_project_id_array ); $i++ ) {
		$t_row = project_cache_row( (int) $c_project_id_array[$i], false, $t_pref->language );
...
function project_cache_all() {
	...
	if( !$g_cache_project_all ) {
		$t_query = "
            ...
            ON a.id = b.id AND b.text_lang = " . db_param() . "
        ";
		$t_result = db_query( $t_query, array( cp_get_language() ) );
...
function project_exists( $p_project_id ) {
	# we're making use of the caching function here. If we succeed in caching
	# the project then it exists and is
	# now cached for use by later function calls. If we can't cache it we
	# return false.
 ...
 * @param integer $p_view_state The view state of the project - public or
 * private.
 * @param string $p_file_path The attachment file path for the project, if not
 * storing in the database.
 * @param boolean $p_enabled Whether the project is enabled.
 * @param boolean $p_inherit_global Whether the project inherits global
 * categories.
 * @return integer
 */
...
function project_delete( $p_project_id ) {
	...
	# Delete all project translations
	cp_drop_project_localisation_by_project( $p_project_id );
 ...
 * @return void
 */
function project_update( $p_project_id, $p_language, $p_name, $p_description, $p_status, $p_view_state, $p_file_path, $p_enabled, $p_inherit_global ) {
	...
	if( strcmp( $p_language, 'default' ) != 0 ) {
		cp_create_project_localisation( $p_project_id, $p_language, $p_name, $p_description );
		...
		$t_query = 'UPDATE {project}
				  ...
				  WHERE id=' . db_param();
		db_query( $t_query, array( (int) $p_status, $c_enabled, (int) $p_view_state, $p_file_path, $c_inherit_global, $p_project_id ) );
		return;
	}
 ...
 * Handles ALL_PROJECTS by returning the internationalized string for All
 * Projects
 *
 * @param integer $p_project_id A project identifier.
 * @param boolean $p_trigger_errors Whether to trigger errors.
 ...
 * if the first parameter is given as 'ALL_PROJECTS', return the global access
 * level (without
...
function project_get_all_user_rows( $p_project_id = ALL_PROJECTS, $p_access_level = ANYBODY, $p_include_global_users = true ) {
	...
	if( $c_project_id != ALL_PROJECTS && $p_include_global_users ) {
		...
		if( VS_PRIVATE == project_get_field( $p_project_id, 'view_state' ) ) {
			# @todo (thraxisp) this is probably more complex than it needs to
			# be
			# When a new project is created, those who meet
			# 'private_project_threshold' are added
			# automatically, but don't have an entry in
			# project_user_list_table.
			...
			if( is_array( $t_private_project_threshold ) ) {
				if( is_array( $p_access_level ) ) {
					# both private threshold and request are arrays, use
					# intersection
					$t_global_access_level = array_intersect( $p_access_level, $t_private_project_threshold );
				} else {
					# private threshold is an array, but request is a number,
					# use values in threshold higher than request
				...
				if( is_array( $p_access_level ) ) {
					# private threshold is a number, but request is an array,
					# use values in request higher than threshold
					$t_global_access_level = array();
					...
					# both private threshold and request are numbers, use
					# maximum
 ...
 * This function involves one more database query than
 * project_update_user_acces() or project_add_user()
 *
 * @param integer $p_project_id A project identifier.
 * @param integer $p_user_id A user identifier.
 ...
 * destination project.
 * The $p_access_level_limit parameter can be used to
