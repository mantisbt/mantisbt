...
function project_table_empty() {
	...
	$t_result = db_query( $t_query, array(), /* limit */ 1 );

	$t_row = db_fetch_array( $t_result );

	if( ! $t_row ) {
		$g_cache_project_missing[(int) $t_row['id']] = true;
		return false;
	} else {
...
function project_update( $p_project_id, $p_name, $p_description, $p_status, $p_view_state, $p_file_path, $p_enabled, $p_inherit_global ) {
	...
	$t_query = "UPDATE {project} SET name=%s, status=%d, enabled=%b, view_state=%d, file_path=%s, description=%s, inherit_global=%b WHERE id=%d";
