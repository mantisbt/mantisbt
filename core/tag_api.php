...
function tag_get_candidates_for_bug( $p_bug_id ) {
	...
	if ( 0 != $p_bug_id ) {
		...
		if ( config_get_global( 'db_type' ) == 'odbc_mssql' ) {
			...
			$t_result = db_query( $query, $t_params );
...
function tag_bug_get_row( $p_tag_id, $p_bug_id ) {
	...
	$t_result = db_query( $query, array( $p_tag_id, $p_bug_id ) );
...
function tag_bug_get_attached( $p_bug_id ) {
	$query = "SELECT t.*, b.user_id as user_attached, b.date_attached
					...
					LEFT JOIN {bug_tag} as b
						on t.id=b.tag_id
					WHERE b.bug_id=%d";
	$t_result = db_query( $query, array( $p_bug_id ) );
...
function tag_get_bugs_attached( $p_tag_id ) {
	...
	$t_result = db_query( $query, array( $p_tag_id ) );
...
function tag_stats_attached( $p_tag_id ) {
	...
	$t_result = db_query( $query, array( $c_tag_id ) );
...
function tag_stats_related( $p_tag_id, $p_limit = 5 ) {
	...
	$t_result = db_query( $query, array( /*query*/ $p_tag_id, /*subquery*/ $c_user_id, $c_user_id, $p_tag_id ) );
