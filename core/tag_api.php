...
function tag_bug_is_attached( $p_tag_id, $p_bug_id ) {
	$t_query = 'SELECT bug_id FROM {bug_tag} WHERE tag_id=%d AND bug_id=%d';
...
function tag_bug_get_row( $p_tag_id, $p_bug_id ) {
	$t_query = 'SELECT * FROM {bug_tag} WHERE tag_id=%d AND bug_id=%d';
	$t_result = db_query( $t_query, array( $p_tag_id, $p_bug_id ) );
...
function tag_get_bugs_attached( $p_tag_id ) {
	$t_query = 'SELECT bug_id FROM {bug_tag} WHERE tag_id=%d';
	$t_result = db_query( $t_query, array( $p_tag_id ) );
...
function tag_stats_attached( $p_tag_id ) {
	...
	$t_query = 'SELECT COUNT(*) FROM {bug_tag} WHERE tag_id=%d';
	$t_result = db_query( $t_query, array( $c_tag_id ) );
...
function tag_stats_related( $p_tag_id, $p_limit = 5 ) {
	...
	$t_query = "SELECT * FROM {bug_tag} WHERE tag_id != %d AND bug_id IN ( $subquery ) ";

	$t_result = db_query( $t_query, array( /*query*/ $p_tag_id, /*subquery*/ $c_user_id, $c_user_id, $p_tag_id ) );
