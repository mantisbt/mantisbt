 ...
 */
function config_process_complex_value( $p_value, $p_trim_quotes = false ) {
	...
	if( preg_match( '/' . $s_regex_array . '/s', $t_value, $t_match ) === 1 ) {
		...
		if( preg_match_all( '/' . $s_regex_element . '/', $t_match[1], $t_elements ) ) {
			foreach( $t_elements[0] as $t_key => $t_element ) {
				...
				if( !empty( $t_split[0] ) ) {
					# associative array
					$t_new_key = config_constant_replace( trim( $t_split[1][0], " \t\n\r\0\x0B\"'" ) );
					$t_new_value = config_process_complex_value( $t_split[2][0], true );
					$t_processed[$t_new_key] = $t_new_value;
				} else {
					# regular array
					$t_new_value = config_process_complex_value( $t_element, true );
		...
		if ( strpos( $t_value, "'" ) === 0 ) {
			...
			$t_value = config_constant_replace( $t_value );
		}
 ...
 * @return array
 */
function config_special_split ( $p_string ) {
	$t_values = array();
	$t_array_element = '';
	$t_paren_level = 0;
 ...
 */
function config_constant_replace( $p_name ) {
