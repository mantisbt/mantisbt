...
class VersionUpdateCommand extends Command {
    ...
    function validate() {
        ...
        $t_name = helper_parse_id( $this->query( 'version_id' ), 'version_id' );
        if( is_blank( $t_name ) ) {
            throw new ClientException( 'Invalid version id', ERROR_EMPTY_FIELD, array( 'id' ) );
        }
    }
    ...
    protected function process() {
        $t_project_id = helper_parse_id( $this->query( 'project_id' ), 'project_id' );
        $t_id= helper_parse_id( $this->query( 'version_id' ), 'version_id' );
        ...
        $t_version = version_get($t_id);
        $t_version->version = $t_name;
        $t_version->description = $t_description;
        $t_version->released = $t_released? VERSION_RELEASED : VERSION_FUTURE;
        $t_version->obsolete = $t_obsolete;
        
        $t_version_id = version_update($t_version);
