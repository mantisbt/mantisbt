...
class VersionRemoveCommand extends Command {
	...
	function validate() {		
		...
		$t_name = $this->payload( 'version_id' );
		if( is_blank( $t_name ) ) {
			throw new ClientException( 'Invalid version id', ERROR_EMPTY_FIELD, array( 'version_id' ) );
		}
	}
	...
	protected function process() {
		...
		$t_id = trim( $this->payload( 'version_id' ) );
		//$t_idNew = trim( $this->payload( 'idNew' ) );
		
		$t_version_id = version_remove($t_id, null);
