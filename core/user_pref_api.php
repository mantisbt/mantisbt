...
function user_pref_insert( $p_user_id, $p_project_id, $p_prefs ) {
	...
	$t_query = 'INSERT INTO {user_pref} (user_id, project_id, ' . $t_vars_string . ') ' .
...
function user_pref_update( $p_user_id, $p_project_id, $p_prefs ) {
	...
	$t_query = 'UPDATE {user_pref} SET ' .$t_pairs_string . ' WHERE user_id=%d AND project_id=%d';
	db_query( $t_query, $t_values );
