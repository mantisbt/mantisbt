...
function profile_create( $p_user_id, $p_platform, $p_os, $p_os_build, $p_description ) {
	...
	# Add profile
	$t_query = "INSERT INTO {user_profile} ( user_id, platform, os, os_build, description )
				  VALUES ( %d, %s, %s, %s, %s )";
...
function profile_delete( $p_user_id, $p_profile_id ) {
	...
	# Delete the profile
	$t_query = 'DELETE FROM {user_profile} WHERE id=%d AND user_id=%d';
...
function profile_update( $p_user_id, $p_profile_id, $p_platform, $p_os, $p_os_build, $p_description ) {
	...
	# Add item
	$t_query = "UPDATE {user_profile} SET platform=%s, os=%s, os_build=%s, description=%s
...
function profile_get_all_rows( $p_user_id, $p_all_users = false ) {
	...
	$t_query = 'SELECT * FROM {user_profile} WHERE user_id IN (' . implode( ',', $t_users ) . ') ORDER BY platform, os, os_build';
...
function profile_get_default( $p_user_id ) {
	$t_query = 'SELECT default_profile FROM {user_pref} WHERE user_id=%d';
