...
function db_connect( $p_dsn, $p_hostname = null, $p_username = null, $p_password = null, $p_database_name = null, $p_db_options = null ) {
	...
	$t_db = MantisDatabase::GetInstance($t_db_type);

	$t_result = $t_db->connect( $p_dsn, $p_hostname, $p_username, $p_password, $p_database_name, $p_db_options );
	...
	$t_db->SetPrefixes( $t_prefix, $t_suffix );
	$g_db_connected = true;
	return true;
}
...
function db_is_mysql() {
	return (MantisDatabase::GetInstance()->GetDbType() == 'mysql');
}
...
function db_is_pgsql() {
	return (MantisDatabase::GetInstance()->GetDbType() == 'postgres');
}
...
function db_is_mssql() {
	return (MantisDatabase::GetInstance()->GetDbType() == 'mssql');
}
...
function db_query( $p_query, $p_params = null, $p_limit = -1, $p_offset = -1 ) {
	if(( $p_limit != -1 ) || ( $p_offset != -1 ) ) {
		$t_result = MantisDatabase::GetInstance()->SelectLimit( $p_query, $p_limit, $p_offset, $p_params );
	} else {
		$t_result = MantisDatabase::GetInstance()->Execute( $p_query, $p_params );
	}
	
	$t_count = $t_result->columnCount();
	$t_meta = array();
	for( $i = 0; $i < $t_count; $i++ ) {
		$t_meta[] = $t_result->getColumnMeta($i);
	}

	if( !$t_result ) {
		throw new MantisBT\Exception\Database\QueryFailed( array( 1,1,$p_query ) );
	} else {
		return array( 'result' => $t_result, 'meta' => $t_meta );
	}
}
...
function db_last_error() {
	return MantisDatabase::GetInstance()->GetLastError();
}
...
function db_fetch_array( &$p_result ) {
	$t_result = $p_result['result']->fetch( PDO::FETCH_ASSOC );
	$i = 0;
	if( is_array( $t_result ) ) {
		foreach ($t_result as $t_key => $t_value) { 
			if( $t_result[$t_key] !== null ) {
				switch( $p_result['meta'][$i]['sqlsrv:decl_type'] ) {
					case 'nvarchar':
						break;
					case 'int':
					case 'int identity':
					case 'tinyint':
					case 'smallint':
						$t_result[$t_key] = (int)$t_result[$t_key];
						break;
					case 'bit':
						$t_result[$t_key] = (bool)$t_result[$t_key];
						break;
					default:
						throw new MantisBT\Exception\Database\QueryFailed( $p_result['meta'][$i]['sqlsrv:decl_type'] );
				}
			}
			$i++;
		}
...
function db_result( $p_result, $p_index1 = 0 ) {
	$t_result = $p_result['result']->fetchColumn($p_index1);
	if( $t_result == null ) {
		return $t_result;
	}
	switch( $p_result['meta'][$p_index1]['sqlsrv:decl_type'] ) {
		case 'nvarchar':
			return $t_result;
		case 'int':
		case 'int identity':
		...
		case 'smallint':
			return (int)$t_result;
		default:
			throw new MantisBT\Exception\Database\QueryFailed( $p_result['meta'][$p_index1]['sqlsrv:decl_type'] );
	}
}
...
function db_insert_id( $p_table = null, $p_field = "id" ) {
	return MantisDatabase::GetInstance()->GetInsertId( $p_table, $p_field );
}
...
function db_table_exists( $p_table_name ) {
	...
	$p_table_name = MantisDatabase::GetInstance()->GetTableName($p_table_name);
...
function db_index_exists( $p_table_name, $p_index_name ) {
	...
	return MantisDatabase::GetInstance()->IndexExists( $p_table_name, $p_index_name );
}
...
function db_field_names( $p_table_name ) {
	$columns = MantisDatabase::GetInstance()->GetColumns( $p_table_name );
...
function db_get_table_list() {
	return MantisDatabase::GetInstance()->GetTables();
}