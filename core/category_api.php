...
function category_remove_all( $p_project_id, $p_new_category_id = 0 ) {
	...
	# Delete translated categories
	cp_drop_category_localisation_by_project( $p_project_id );
...
function category_get_row( $p_category_id, $p_error_if_not_exists = true ) {
	...
	$t_params = array( $t_language, $p_category_id );
	...
	$t_query = "
        SELECT
            ...
            a.status
        FROM {category} a
        LEFT JOIN {category_localized} b
 ...
 * If the current project is ALL_PROJECTS get all categories for all accessible
 * projects.
