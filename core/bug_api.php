...
class BugData {
	...
	function create() {
		...
		if( 0 == $this->handler_id ) {
			...
			$t_query = 'SELECT user_id FROM {category} WHERE id=%d';
		...
		db_query( $t_query, array( $this->project_id, $this->reporter_id, $this->handler_id, 
								   $this->duplicate_id, $this->priority, $this->severity, 
								   $this->reproducibility, $t_status, $this->resolution, 
								   $this->projection, $this->category_id, $this->date_submitted, 
								   $this->last_updated, $this->eta, $t_text_id, $this->os, 
								   $this->os_build, $this->platform, $this->version, $this->build, 
								   $this->profile_id, $this->summary, $this->view_state, 
								   $this->sponsorship_total, $this->sticky, $this->fixed_in_version, 
	...
	function update( $p_update_extended = false, $p_bypass_mail = false ) {
		...
		if( $p_update_extended ) {
			...
			$t_query = "UPDATE {bug_text} SET description=%s, steps_to_reproduce=%s, additional_information=%s WHERE id=%d";
...
function bug_copy( $p_bug_id, $p_target_project_id = null, $p_copy_custom_fields = false, $p_copy_relationships = false, $p_copy_history = false, $p_copy_attachments = false, $p_copy_bugnotes = false, $p_copy_monitoring_users = false ) {
	...
	if( $p_copy_custom_fields ) {
		$t_query = "SELECT field_id, bug_id, value FROM {custom_field_string} WHERE bug_id=%d";
		...
		while( $t_bug_custom = db_fetch_array( $t_result ) ) {
			...
			$t_query = 'INSERT INTO {custom_field_string} ( field_id, bug_id, value ) VALUES (%d, %d, %s)';
	...
	# Copy bugnotes
	if( $p_copy_bugnotes ) {
		$t_query = 'SELECT * FROM {bugnote} WHERE bug_id=%d';
		$t_result = db_query( $t_query, array( $t_bug_id ) );
	...
	if( $p_copy_history ) {
		...
		$t_query = 'SELECT * FROM {bug_history} WHERE bug_id = %d';
		$t_result = db_query( $t_query, array( $t_bug_id ) );
...
function bug_get_attachments( $p_bug_id ) {
	...
	$db_result = db_query( $t_query, array( $p_bug_id ) );
...
function bug_set_field( $p_bug_id, $p_field_name, $p_value ) {
	...
	# Update fields
	$t_query = 'UPDATE {bug} SET ' . $p_field_name . '=%s WHERE id=%d';
...
function bug_assign( $p_bug_id, $p_user_id, $p_bugnote_text = '', $p_bugnote_private = false ) {
	...
	if(( $t_ass_val != $h_status ) || ( $p_user_id != $h_handler_id ) ) {
		...
		# get user id
		$t_query = 'UPDATE {bug} SET handler_id=%d, status=%d WHERE id=%d';
...
function bug_monitor( $p_bug_id, $p_user_id ) {
	...
	$t_query = 'INSERT INTO {bug_monitor} ( user_id, bug_id ) VALUES (%d,%d)';
...
function bug_monitor_copy( $p_source_bug_id, $p_dest_bug_id ) {
	...
	$t_query = 'SELECT user_id FROM {bug_monitor} WHERE bug_id = %d';
...
function bug_unmonitor( $p_bug_id, $p_user_id ) {
	...
	$t_query = 'DELETE FROM {bug_monitor} WHERE bug_id=%d';
	$db_query_params[] = $p_bug_id;

	if( $p_user_id !== null ) {
		$t_query .= " AND user_id=%d";
		$db_query_params[] = $p_user_id;
	}

	db_query( $t_query, $db_query_params );
