...
class BugData {
	...
	private function bug_get_bugnote_count() {
		...
		$query = "SELECT COUNT(*) FROM {bugnote}
					  WHERE bug_id =" . db_param() . " $t_restriction";
		$t_result = db_query( $query, array( $this->id ) );
	...
	function create() {
		...
		db_query( $query, array( $this->description, $this->steps_to_reproduce, $this->additional_information ) );
		...
		if( 0 == $this->handler_id ) {
			...
			$query = "SELECT user_id FROM {category}
						  WHERE id=" . db_param();
			$t_result = db_query( $query, array( $this->category_id ) );
...
function bug_cache_row( $p_bug_id, $p_trigger_errors = true ) {
	...
	$query = "SELECT * FROM {bug} WHERE id=%d";
...
function bug_copy( $p_bug_id, $p_target_project_id = null, $p_copy_custom_fields = false, $p_copy_relationships = false, $p_copy_history = false, $p_copy_attachments = false, $p_copy_bugnotes = false, $p_copy_monitoring_users = false ) {
	...
	if( $p_copy_custom_fields ) {
		...
		$t_result = db_query( $query, array( $t_bug_id ) );
	...
	if( $p_copy_bugnotes ) {
		...
		while( $t_bug_note = db_fetch_array( $t_result ) ) {
			...
			$t_result2 = db_query( $query2, array( $t_bugnote_text_id ) );

			$t_bugnote_text_insert_id = -1;
			if( $t_bugnote_text = db_fetch_array( $t_result2 ) ) {
				$query2 = "INSERT INTO {bugnote_text}
							   ( note )
							   VALUES ( " . db_param() . ' )';
				db_query( $query2, array( $t_bugnote_text['note'] ) );
			...
			$query2 = "INSERT INTO {bugnote}
						   ...
						   VALUES ( " . db_param() . ",
						   			...
						   			" . db_param() . ",
						   			" . db_param() . ')';
			db_query( $query2, array( $t_new_bug_id, $t_bug_note['reporter_id'], $t_bugnote_text_insert_id, $t_bug_note['view_state'], $t_bug_note['date_submitted'], $t_bug_note['last_modified'] ) );
		}
	}
...
function bug_delete( $p_bug_id ) {
	...
	db_query( $query, array( $t_bug_text_id ) );

	# Delete the bug entry
	$query = "DELETE FROM {bug} WHERE id=%d";
	db_query( $query, array( $c_bug_id ) );
...
function bug_monitor_copy( $p_source_bug_id, $p_dest_bug_id ) {
	...
	$t_result = db_query( $query, array( $c_source_bug_id ) );
