...
$highlight_attribute = config_get( 'highlight_attribute' );
$t_attribute_string = config_get( $highlight_attribute . '_enum_string' );
$t_attributes = MantisEnum::getAssocArrayIndexedByValues( $t_attribute_string );
$t_colors = config_get( $highlight_attribute . '_colors' );
...
foreach( $t_attributes AS $t_id=>$t_label ) {
	if( array_key_exists( $t_label, $t_colors ) ) {
		echo "." . string_css($t_label) . "-color { background-color: {$t_colors[$t_label]}; width: $t_color_width%; }\n";
	}
