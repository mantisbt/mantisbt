...
$t_show_votes = in_array( 'votes', $t_fields );
$t_votes = $t_show_votes ? $t_bug->votes : '';
...
if( $t_show_votes || $t_show_status || $t_show_resolution ) {
	echo '<tr>';
	...
	# Count / votes
	if( $t_show_votes ) {
		echo '<th class="bug-votes category">', lang_get( 'count' ), '</th>';
		echo '<td class="bug-votes">', $t_votes, '</td>';
	} else {
		$t_spacer += 2;
	}
