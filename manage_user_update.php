...
$t_old_password = $t_user['password'];
...
# password has been changed, we should hash it
$t_password_changed = false;
if( 0 != strcasecmp( $t_old_password, $f_password ) ) {
	$t_password_changed = true;
...
if( $f_protected && $t_old_protected ) {
	$t_query = 'UPDATE {user}
			...
			WHERE id=' . db_param();
	$t_query_params = array( $c_username, $c_email, $c_password, $c_protected, $c_realname, $c_user_id );
	...
	$t_query = 'UPDATE {user}
			SET username=' . db_param() . ', email=' . db_param() . ',
				...
				protected=' . db_param() . ', realname=' . db_param() . '
			WHERE id=' . db_param();
	$t_query_params = array( $c_username, $c_email, $c_password, $c_access_level,
		$c_enabled, $c_protected, $c_realname, $c_user_id );
}
