...
if( $f_add_and_edit ) {
	$t_redirect_url = helper_mantis_url( 'manage_proj_cat_edit_page.php?category_id=' . $t_id . '&project_id=' . $f_project_id );
} else if( $f_project_id == ALL_PROJECTS ) {
	$t_redirect_url = helper_mantis_url('manage_proj_page.php');
} else {
	$t_redirect_url = helper_mantis_url( 'manage_proj_edit_page.php?project_id=' . $f_project_id . '#categories' );
}
