	...
	switch( $f_action ) {
		case 'CLOSE':
			...
			if( access_can_close_bug( $t_bug ) ) {
				if( ( $t_status < $t_closed ) &&
					...
					$t_bugnote['bugnote_text']	 = $f_bug_notetext;
					$t_bugnote['private']		 = $f_bug_noteprivate;

					$t_issue = array(
								  'last_updated'	 => $t_bug->last_updated,
								  'status'	 => array( 'id' => config_get( 'bug_closed_status_threshold' ) ),
					);

					$t_data = array(
								  'payload' => array(
											    'issue'	 => $t_issue,
											    'notes'	 => array( 0 => $t_bugnote ),
								  ),
					);

					$t_data['options'] = array(
								  'action_type' => BUG_UPDATE_TYPE_CLOSE,
					);
			...
			if( access_has_bug_level( config_get( 'update_bug_assign_threshold', config_get( 'update_bug_threshold' ) ), $t_bug_id ) ) {
				if( access_has_bug_level( config_get( 'handle_bug_threshold' ), $t_bug_id, $f_assign ) ) {
					if( bug_check_workflow( $t_status, $t_assign_status ) ) {
						...
						$t_issue = array(
									  'last_updated'	 => $t_bug->last_updated,
									  'status'	 => array( 'id' => $t_assign_status ),
									  'handler'	 => array( 'id' => $f_assign ),
						);
						...
						$t_data['options'] = array(
									  'action_type' => BUG_UPDATE_TYPE_ASSIGN,
						);
		...
		case 'RESOLVE':
			...
			if( access_has_bug_level( access_get_status_threshold( $t_resolved_status, $t_bug->project_id ), $t_bug_id ) ) {
				if( ( $t_status < $t_resolved_status ) &&
					...
					$t_issue = array(
								  'last_updated'		 => $t_bug->last_updated,
								  'status'		 => array( 'id' => config_get( 'bug_resolved_status_threshold' ) ),
								  'handler'		 => array( 'id' => $t_handler_id ),
								  'resolution'		 => array( 'id' => $f_resolution ),
								  'fixed_in_version'	 => $f_fixed_in_version,
					);
					...
					$t_data['options'] = array(
								  'action_type' => BUG_UPDATE_TYPE_CHANGE_STATUS,
					);
		...
		case 'UP_CATEGORY':
			...
			if( access_has_bug_level( config_get( 'update_bug_threshold' ), $t_bug_id ) ) {
				if( category_exists( $f_category_id ) ) {
					$t_issue = array(
								  'last_updated'	 => $t_bug->last_updated,
								  'category'	 => array( 'id' => $f_category_id ),
					);
		...
		case 'UP_TARGET_VERSION':
			...
			if( access_has_bug_level( config_get( 'roadmap_update_threshold' ), $t_bug_id ) ) {
				if( $f_target_version === '' || version_get_id( $f_target_version, $t_bug->project_id ) !== false ) {
					$t_issue = array(
								  'last_updated'	 => $t_bug->last_updated,
								  'target_version' => $f_target_version,
					);
		...
		case 'VIEW_STATUS':
			if( access_has_bug_level( config_get( 'change_view_status_threshold' ), $t_bug_id ) ) {
				...
				$t_issue	 = array(
							  'last_updated'	 => $t_bug->last_updated,
							  'view_state'	 => array( 'id' => $f_view_status ),
				);
		...
		case 'CUSTOM':
			...
			$t_custom_field[]	 = array(
						  'field'	 => array( 'id' => $f_custom_field_id ),
						  'value'	 => $t_custom_field_value,
			);

			$t_issue = array(
						  'last_updated'	 => $t_bug->last_updated,
						  'custom_fields'	 => $t_custom_field,
			);
