<?php
	...
	# USAGE: set #debug_timer to ON and just call
	# $g_timer->mark_time( 'token name' );
		...
		function print_times() {
			...
			if ( ON == $debug_timer ) {
				for ($i=0;$i+1<count($this->atime);$i++) {
					...
					PRINT '<span class="italic">Time: '.number_format( $time, 5 ).' seconds for '.$this->atime[$i][0].' -to- '.$this->atime[$i+1][0].'</span><br />';
				}
			} else {
